...
const WindowManagement = new (function () {
  ...
  this.windowIsVisible = function (window) {
    return !(
      window.classList.contains("closed") || window.classList.contains("exited")
    );
  };
  ...
  this.getWindow = function (windowString) {
    ...
    if (!w)
      throw new ReferenceError(
        `Document element ${windowString} does not exist.`
      );
    if (!w.classList.contains("windowPositioner"))
      throw new ReferenceError(
        `Document element ${windowString}}} is not a window.`
      );
    return w;
  };
  this.bringWindowToTop = function (window) {
    ...
    window.style.zIndex =
      (this.windows.length - 1) * this.LAYERS_BETWEEN_SCREENS;
  };
  ...
  this.exitWindow = function (windowString) {
    ...
    if (window.classList.toggle("exited")) {
      // if screen has been exited
      window.classList.remove("closed");
      if (toolbarbutton) toolbarbutton.classList.add("exited");
    } else {
      // if screen has been turned on
  ...
  this.openWindow = function (windowString) {
    let window = this.getWindow(windowString);
    if (window.classList.contains("exited")) {
      // window is coming out of an exited state
      ...
      if (window.classList.contains("closed")) {
        ...
        window.classList.remove("closed");
        setTimeout(() => {
          window.classList.remove("smoothTopAnimation");
        }, this.WINDOW_OPEN_SPEED_MS);
      } else {
        // if window was open
  ...
  for (let i = 0; i < this.windows.length; i++) {
    this.windows[i].style.setProperty(
      "z-index",
      i * this.LAYERS_BETWEEN_SCREENS
    );
  }
  // let the windows be moved around
  for (let window of this.windows) {
    window.addEventListener("mousedown", (e) => {
      this.DragWindow.dragStart(window);
    });
  ...
  document.addEventListener("mousemove", (e) => {
    this.DragWindow.dragTo(e);
  });
  document.addEventListener("touchmove", (e) => {
    this.DragWindow.dragTo(e);
  });
  document.addEventListener("mouseup", () => {
    this.DragWindow.dragEnd();
  });
...
const BlossomManagement = new (function () {
  ...
  this.setBlossomWindow = function () {
    let blossomWindowNum = Math.floor(
      Math.random() * WindowManagement.windows.length
    );
    for (let i = 0; i < WindowManagement.windows.length; i++) {
      ...
      let potentialWindow =
        WindowManagement.windows[
          (blossomWindowNum + i) % WindowManagement.windows.length
        ];
  ...
  this.createNewBlossom = function () {
    ...
    let { x, y, width, height } = this.blossomWindow
    ...
    newPetal.style.zIndex =
      parseInt(this.blossomWindow.style.getPropertyValue("z-index")) + 1;
    newPetal.style.top = Math.floor(Math.random() * height) + y + "px";
    newPetal.style.left = Math.floor(Math.random() * width) + x + "px";
    newPetal.style.setProperty(
      "--gs",
    ...
    newPetal.style.setProperty(
      "--ad1",
      this.ANIMATION_DIRECTIONS[
        Math.floor(Math.random() * this.ANIMATION_DIRECTIONS.length)
      ]
    );
    newPetal.style.setProperty(
      "--ad2",
  ...
  this.blossomGenerationInterval = setInterval(() => {
    if (
      this.liveBlossoms < this.activeBlossoms &&
      WindowManagement.windowIsOpen()
    ) {
      this.blossomsLeft--;
      if (
        this.blossomsLeft <= 0 ||
        !WindowManagement.windowIsVisible(this.blossomWindow)
      ) {
        this.setBlossomWindow();
        this.blossomsLeft = Math.floor(
          Math.random() * this.activeBlossoms * this.MAX_BLOSSOM_CYCLES
        );
      }
      this.liveBlossoms++;
      setTimeout(() => {
        this.createNewBlossom();
...
const ClockManagement = new (function () {
  ...
  this.updateTime = function () {
    ...
    let time = `${d.getHours() % 12 || 12}:${d
      .getMinutes()
      .toLocaleString("en-US", { minimumIntegerDigits: 2 })} ${meridiem}`;
...
const TerminalHelpers = new (function () {
  ...
  this.terminalContent = document
    .querySelector("#Terminal")
    .querySelector(".terminalContent");
  ...
  this.putOutError = function (string) {
    let errorString =
      "<span style='color:var(--red);'</b>" + string + "</span>";
  ...
  this.putOutWarning = function (string) {
    let errorString =
      "<span style='color:var(--yellow);'</b>" + string + "</span>";
  ...
  this.availableCommands = [
    this.Command(
      ...
      (args) => {
        ...
        for (let i = 0; i < this.availableCommands.length; i++) {
          ...
          let argString = "";
          if (c.args) {
            argString += ` <span style="color: var(--yellow)">${c.args.join(
              " "
            )}</span>`;
          }
          rows += `<tr><td>&gt; <b style="color: var(--purple)">${c.name}${argString}</b></td><td>-</td><td>${c.desc}</td></tr>`;
          if (i < this.availableCommands.length - 1)
            rows += "<tr class='spacerRow'><td></td><td></td><td></td></tr>";
        }
        rows += "</table>";
    ...
    this.Command(
      "petals",
      ["[count:number[0-" + BlossomManagement.MAX_TOTAL_BLOSSOMS + "]]"],
      "changes the max number of petals",
      (args) => {
        if (!args) {
          this.putOutHTML(
            "Max petals: " +
              BlossomManagement.activeBlossoms +
              ", Active petals: " +
              BlossomManagement.liveBlossoms
          );
          return;
        }
        args = args.split(" ");
        if (args.length > 1)
          this.putOutWarning("petals only accepts one argument");
        ...
        if (arg > BlossomManagement.MAX_TOTAL_BLOSSOMS || arg < 0) {
          this.putOutError(
            "Number is not within range 0-" +
              BlossomManagement.MAX_TOTAL_BLOSSOMS
          );
          return;
        }
    ...
    this.Command(
      "refresh",
      null,
      "reverts windows back to their original positions",
      (args) => {
    ...
    this.Command(
      "ps",
      null,
      "views the process status of the currently running processes",
      (args) => {
        if (args)
          this.putOutWarning("Arguments are not supported for ps command");
        let HTMLstring = "<table>";
        HTMLstring +=
          "<tr><th>PID</th><th>Process Name</th><th>Status</th></tr>";
        for (let i = 0; i < WindowManagement.windows.length; i++) {
    ...
    this.Command(
      "kill",
      ["[pid:number]"],
      "closes a currently running process",
      (args) => {
        if (!args) {
        ...
        args = args.split(" ");
        if (args.length > 1)
          this.putOutWarning("kill only accepts one argument");
        let arg = parseInt(args[0]);
        if (isNaN(arg)) {
          this.putOutError("Argument 1 is not a valid number");
          return;
        ...
        if (
          !WindowManagement.windows[arg] ||
          WindowManagement.windows[arg].classList.contains("exited")
        ) {
  ...
  this.runCommand = function (inputText) {
    ...
    this.putOutError(
      "Command <span style='color: var(--yellow)'>" +
        inputText +
        "</span> not found. Try <b style='color: var(--purple)'>help</b>."
    );
    return;
  };
...
const BrowserHelpers = new (function () {
  ...
  this.updateContentScroll = function () {
    ...
    let newHeight =
      browserFrame.getBoundingClientRect().height - subtractHeight + "px";
  ...
  for (let i = 0; i < this.browserContent.children.length; i++) {
    ...
    element.style.animationDelay = this.FADE_IN_WAIT + 0.1 * i + "s";
  }
  let cards = document.querySelector(".browserContent>.cards").children;
  console.log(cards);
  for (let i = 0; i < cards.length; i++) {
    let card = cards[i];
    console.log(card, this.FADE_IN_WAIT * 2 + 0.1 * i + "s");
    card.style.animationDelay = this.FADE_IN_WAIT * 1.5 + 0.1 * i + "s";
  }
